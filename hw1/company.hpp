...
private:
  ...
  // Finds largest company a student is apart of
  Company* largestCompany(int i);

  // Checks if student is in range
  bool inRange(int i);
  
  // Vector stores large companies
  std::vector <Company*> bigComp;

  // Prints list of companies
  void printList(Company* comp);

  // DFS algortihm for split to find smallest index
  int dfs(Company* comp);

  void realPrint();

  // Delete Dynamically Allocated new companies
  void clearMem(Company* comp);
